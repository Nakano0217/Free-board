...
(function(){
  ...
  function setInverted(on, save = true){
    const root = document.documentElement;
    if(on){
      root.classList.add('inverted');
      if(invertBtn){ invertBtn.setAttribute('aria-pressed','true'); invertBtn.classList.add('active'); }
    } else {
      root.classList.remove('inverted');
      if(invertBtn){ invertBtn.setAttribute('aria-pressed','false'); invertBtn.classList.remove('active'); }
    }
    if(save){
      try{ localStorage.setItem('inverted', on ? '1' : '0'); }catch(_){ }
    }
  }

  if(invertBtn){
    invertBtn.addEventListener('click', ()=> setInverted(!document.documentElement.classList.contains('inverted')));
  }

  // 保存された設定の復元
  try{
    const saved = localStorage.getItem('inverted');
    if(saved === '1') setInverted(true, false);
  }catch(_){ }
  ...
  window._boogie = { clear: () => clearBtn.click(), setTool, setInverted };
})();
